...
__global__ void flash_attention_ptx_kernel(
    ...
    // So warps 0,1 handle mi=0 (rows 0-15), warps 2,3 handle mi=1 (rows 16-31), etc.
    //
    // This means for softmax, warps 0&1 share rows 0-15, warps 2&3 share 16-31, etc.
    // The softmax row state must be shared between warp pairs.
    // Each thread in warp w knows its rows: (lane_id/4)%8 and (lane_id/4)%8 + 8
    // within the mi tile. Global rows: mi*16 + local_row.
    //
    // For O output [BLOCK_M, D_HEAD]: same mi assignment.
    // Warp 2w handles di=0..3, warp 2w+1 handles di=4..7 for mi=w.

    constexpr int MI_PER_WARP_PAIR = 1;  // Each pair of warps handles 1 M-tile (16 rows)
    ...
    for (int kv_tile = 0; kv_tile < num_kv_tiles; kv_tile++) {
        ...
        //
        // ldmatrix.trans layout for B (col-major, effectively 16×8 → transposed load):
        ...
        //   to rearrange for MMA's col-major B format
        {
            ...
            for (int ni_local = 0; ni_local < QK_TILES_PER_WARP; ni_local++) {
                int ni = warp_half * QK_TILES_PER_WARP + ni_local;  // n-tile in m16n8 space
                ...
                for (int ki = 0; ki < TILES_K; ki++) {
                    ...
                    uint32_t a0, a1, a2, a3;
                    {
                        ...
                        const half* addr = smem_q + (mi * 16 + row) * Q_STRIDE + ki * 16 + col;
                        ldmatrix_x4(a0, a1, a2, a3, addr);
                    }

                    // Load B tile (K^T): need 16×8 col-major = K[ni*8..ni*8+7, ki*16..ki*16+15]^T
                    // K is stored row-major in smem_k as [BLOCK_N rows, D_HEAD cols]
                    // K^T col j = K row j, so col-major B means:
        ...
        // O accumulator using the known MMA layout.
        {
            ...
            for (int r = 0; r < ROWS_PER_WARP; r++) {
                ...
                // (defined outside this loop — using smem_s region which we're done with)
        ...
        //   → V stored row-major with stride KV_STRIDE means columns are contiguous? No.
        //   → V[n][d] at smem_v[n * KV_STRIDE + d], stride KV_STRIDE between rows
        //   → For ldmatrix.trans: load V[di*8 + t%8, ki*16 + ...] and transpose
        //
        ...
        //   B[k,j] = V[n, d] → col-major means stride 1 between k (n) at fixed j (d)
        //   V[n, d] at smem_v[n * KV_STRIDE + d], stride KV_STRIDE between consecutive n
        //   This is NOT col-major (col-major would need stride 1 between rows).
        //   It's row-major with leading dim KV_STRIDE.
        //
        ...
        //     B = V (row-major in smem, loaded with ldmatrix.trans → col-major regs)
        //
...

        {
            ...
            for (int di_local = 0; di_local < PV_TILES_PER_WARP; di_local++) {
                int di = warp_half * PV_TILES_PER_WARP + di_local;  // d-tile in m16n8 space
                ...
                for (int ki = 0; ki < TILES_BN; ki++) {
                    ...
                    uint32_t a0, a1, a2, a3;
                    {
                        ...
                        const half* addr = smem_p + (mi * 16 + row) * P_STRIDE + ki * 16 + col;
                        ldmatrix_x4(a0, a1, a2, a3, addr);
                    }
    ...
    //     col1 = col0 + 1
    {
        ...
        for (int di_local = 0; di_local < PV_TILES_PER_WARP; di_local++) {
            ...
            float inv_sum0 = (smem_rsum[global_row0] > 0.0f) ? (1.0f / smem_rsum[global_row0]) : 0.0f;
            float inv_sum1 = (smem_rsum[global_row1] > 0.0f) ? (1.0f / smem_rsum[global_row1]) : 0.0f;
        ...
        if (lane_id % 4 == 0 && LSE != nullptr) {
            ...
            int gq1 = q_start + global_row1;
            if (gq0 < seq_len)
                LSE[bh_idx * seq_len + gq0] = smem_rmax[global_row0] + logf(fmaxf(smem_rsum[global_row0], 1e-10f));
            if (gq1 < seq_len)
                LSE[bh_idx * seq_len + gq1] = smem_rmax[global_row1] + logf(fmaxf(smem_rsum[global_row1], 1e-10f));
        }
    }
}
